...
class SPOClient {
	...
	*/
	...
	public function getHeaders($options) {
		$url = $this->url . "" . $options['list']; 
        $options['url'] = $url;
        return $this->request($options);
	}


    public function requestImage($options)
    {
        $data = array_key_exists('data', $options) ? json_encode($options['data']) : '';
        $headers = array(
            'Accept: application/json; odata=verbose',
	   ...
	   $docpath = $_SERVER['DOCUMENT_ROOT'] . '/images/avatar/'.$options['picturePath'].'/' . $options['pictureType'] . $options['idRegister'] . '.jpg';
